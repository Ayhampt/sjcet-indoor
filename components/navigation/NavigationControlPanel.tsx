...
export function NavigationControlPanel({
  ...
  useEffect(() => {
    if (!startPoint && currentFloorNodes.length > 0) {
      const centralNode = currentFloorNodes.find(
        (n) => n.label?.includes("Center") || n.label?.includes("Central"),
      );
      if (centralNode) {
        setStartPoint({
  ...
  const handleNodeSelect = (
    ...
    const point: NavigationPoint = {
      ...
      label: isRoom
        ? `Room ${(node as Room).roomNumber}`
        : (node as NavigationNode).label || node.id,
      source: "manual",
      timestamp: Date.now(),
    };
    ...
    <div className="w-full space-y-4">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg p-4 shadow-lg">
        ...
        <p className="text-sm text-blue-100">
          Select starting point and destination to find the optimal route
        </p>
      </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-green-50 to-green-100/50 dark:from-green-900/20 dark:to-green-800/20 border border-green-200 dark:border-green-800 rounded-lg mb-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-green-700 dark:text-green-300">
                {startPoint.source === "current_location"
                  ? "Current Location"
                  : "Selected"}
              </p>
            </div>
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-sm text-green-800 dark:text-green-200">
                Route to <strong>{selectedDestination.name}</strong> (Room{" "}
                {selectedDestination.roomNumber})
              </p>
            </div>
          </div>
