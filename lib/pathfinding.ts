...
interface Graph {
  [key: string]: Array<{ node: NavigationNode; weight: number; edge?: NavigationEdge }>;
}
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (node.requiresStairs && node.nodeType === 'stairs_only') {
      ...
      if (i > 0) {
        ...
        const edge = edges.find(
          (e) => (e.from === prevNodeId && e.to === nodeId) || 
    ...
    if (node.type === 'room') {
      ...
      if (parentJunction && !isAccessible(nodeId, parentJunction, nodeMap)) {
        violations.push(`Room ${nodeId} is not accessible from junction ${parentJunction}`);
      }
    }
  }
...
function findParentJunction(
  ...
  const directJunctions = edges
    .filter((e) => (e.from === roomId || e.to === roomId) && e.type === 'direct')
